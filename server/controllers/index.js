...
// function to create new dog in the database
...
// function to search for a dog by name and increase its age by 1
const findDogByName = async (req, res) => {
  ...
  try {
    ...
    return res.json({ message: 'Dog found, age updated', dog });
  } catch (err) {
    console.error(err);
    return res.status(500).json({ error: 'Error retrieving dog' });
  }
};
